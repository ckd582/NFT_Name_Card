...
contract SimpleCardNFTFactory is ERC721 {
    ...
    struct SimpleCardInfo {
        //내 명함에 들어가는 기본적인 정보들로 구조체(여러 개의 변수를 하나의 단위로 묶어서 관리할 수 있게 해주는 데이터 타입)를 만듦
        //essential
        string name;
        string email;
    ...
    mapping(address => mapping(uint => bool))
        private _isTokenStillOwnedByIssuer; //issuer가 발급한 tokenId들이 현재 issuer에게 있는지. 있으면 true, 없으면 false
    ...
    modifier isSimpleCardInfoRegistered() {
        ...
        require(
            keccak256(abi.encodePacked(mySimpleCardInfo.name)) !=
    ...
    function mintSimpleCardNFT() public payable isSimpleCardInfoRegistered {
        //자신의 명함 NFT 한 개 발급
        tokenId++;
    ...
    function transferSimpleCardNFT(
        address to
    ) public isSimpleCardInfoRegistered {
        require(
            _amountOfTokenOwnedByIssuer[msg.sender] != 0,
            "Mint your Simple Card NFT first"
        );
        ...
        for (uint i = 0; i < tokenIdsMadeByIssuer.length; i++) {
            ...
            if (
                _isTokenStillOwnedByIssuer[msg.sender][_tokenIdMadeByIssuer] ==
                true
            ) {
            ...
            if (
                (i == tokenIdsMadeByIssuer.length - 1) &&
                (_isTokenStillOwnedByIssuer[msg.sender][_tokenIdMadeByIssuer] ==
                    false)
            ) {
    ...
    //getter 함수
    function getSimpleCardInfo(
        address issuer
    ) external view returns (SimpleCardInfo memory) {
        return _infos[issuer];
    }

    function getAmountOfTokenOwnedByIssuer(
        address issuer
